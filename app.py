...
import pandas as pd
from utils.data import load_and_merge_all, save_atribuicao, save_edicao_com_historico, get_edicoes, get_historico

st.set_page_config(page_title="Sistema Gerencial de Convênios", layout="wide")

# --- ESTADO DA SESSÃO ---
if 'main_df' not in st.session_state:
    # Dados de teste embutidos para você testar sem subir planilhas
    st.session_state.main_df = pd.DataFrame({
        ...
        'valor_global_painel': [1000000.0, 500000.0, 200000.0],
        ...
        # Campos de coordenações (vazios para teste)
        'situacao_pb': [pd.NA, pd.NA, pd.NA],
        'analista_pb': [pd.NA, pd.NA, pd.NA],
        'status_analise_pb': [pd.NA, pd.NA, pd.NA],
        'fiscal_exec': [pd.NA, pd.NA, pd.NA],
        'status_exec': [pd.NA, pd.NA, pd.NA],
        'status_acao_convenente': [pd.NA, pd.NA, pd.NA],
        'status_obra': [pd.NA, pd.NA, pd.NA],
        'fiscal_pc': [pd.NA, pd.NA, pd.NA],
        'status_exec_pc': [pd.NA, pd.NA, pd.NA],
        'status_obra_pc': [pd.NA, pd.NA, pd.NA],
        'status_pc': [pd.NA, pd.NA, pd.NA],
    })
...
if df.empty:
    st.warning("Base de dados vazia. Use a aba 'Upload' para carregar dados.")
    st.stop()
...
if menu == "Geral":
    ...
    if submitted or st.session_state.selected_id:
        res = df.copy()
        # Aplicação dos filtros (exemplos básicos)
        if f_inst:
...
# --- PÁGINA: ATRIBUIÇÃO (GESTORES) ---
    ...
    if user_role != "Gestor":
        ...
        inst = st.text_input("Digite o Nº Instrumento ou Proposta")  # Agora campo texto para teste
